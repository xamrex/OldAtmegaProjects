...
ISR(TIMER0_COMP_vect)
{
	...
	if(licznik==16) 		LED_DATA = pgm_read_byte( &cyfry[cy1] );	// gdy zapalony wyœw.1 podaj stan zmiennej c1
	else if(licznik==32) LED_DATA = pgm_read_byte( &cyfry[cy2] );	// gdy zapalony wyœw.2 podaj stan zmiennej c2
	else if(licznik==64) LED_DATA = pgm_read_byte( &cyfry[cy3] );	// gdy zapalony wyœw.3 podaj stan zmiennej c3
	else if(licznik==128) LED_DATA = pgm_read_byte( &cyfry[cy4] );	// gdy zapalony wyœw.4 podaj stan zmiennej c4
	...
	if(licznik>128) licznik = 16;		// jeœli licznik wiêkszy ni¿ 8 to ustaw na 1
}
