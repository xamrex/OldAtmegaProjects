...
uint8_t wylosowana,liczba,flaga,flaga2,powtorzenie;
...
/********************** NACISNIECKIE KLAWISZA UP or DOWN ******************/
		if(Ir_key_press_flag) {								  //Odbieranie kodu z pilota
			if (command==cursor_down && pozycja<2){
				pozycja++; //Dodanie tego warunku, aby pozycja by³a max 2
				_delay_ms(200);
			}
			if (command==cursor_up && pozycja>0){
				pozycja--;   //Dodanie tego warunku, aby pozycja by³a min 0
				_delay_ms(200);
			}
		}
		Ir_key_press_flag=0;
